    ...
    ...) 
{
    ...
    if (missing(lmat) || is.null(lmat)) {
        ...
        if (!missing(RowSideColors)) {
           ...
           #     1), 1), lmat[, 2] + 1)
           lmat <- cbind(lmat[, 1] + 1, lmat[, 2] + 1 , c(rep(NA, nrow(lmat) - 
                1), 1))
            #LAC - changed the width from 0.2 to 0.4 for the RowSideColors column
            #lwid <- c(lwid[1], 0.2, lwid[2])
            lwid <- c(lwid[1], lwid[2], 0.2)
        }
        lmat[is.na(lmat)] <- 0
    }
    ...
    #LAC - changed the side of the plot the axis is to be drawn on
