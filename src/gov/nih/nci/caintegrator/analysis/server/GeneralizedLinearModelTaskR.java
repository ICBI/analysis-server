...
//                    "glmReporters <- dimnames(glmResult)[[1]]").asVector();
            String[] reporterIds = doREval(
                      "glmReporters <- dimnames(glmResult)[[1]]").asStrings();
            ...
            if(glmRequest.getComparisonGroups().size() < 2 && (glmRequest.getCoVariateTypes() == null || glmRequest.getCoVariateTypes().size() < 1)) {
                ...
                String[] groupIds = doREval("glmGroups <- dimnames(glmResult)[[2]]")
                    .asStrings();
                for (int i=0; i < groupIds.length; i++ ) {
                    resultSampleGroups.add(new SampleGroup(groupIds[i]));
            ...
            logger.debug("reporterIds.length=" + reporterIds.length);
